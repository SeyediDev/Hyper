    ...
    ) : ILotteryService
{
    ...
    {
        // Get lottery
        Lottery? lottery = await lotteryRepo.FirstOrDefaultWithIncludeAsync(
            x=> x.Promotion,
        ...
        if (lottery.FromDate.HasValue && lottery.FromDate > DateTime.UtcNow)
            throw new InvalidOperationException("قرعه‌کشی هنوز آغاز نشده است");
        
        if (lottery.ToDate.HasValue && lottery.ToDate < DateTime.UtcNow)
        ...
        CustomerTenant customerTenant = await customerTenantService.GetOrCreateAndSetReservedAttributesAsync(
            lottery.Promotion.TenantId, request.CustomerId, null/*TODO*/, cancellationToken);
        ...
        if (lottery.RequiresPayment && lottery.PaymentPointId.HasValue && lottery.PaymentAmount.HasValue)
        {
            ...
            var customerBalances = await customerTenantService.GetPointBalancesAsync(customerTenant.Id, false, cancellationToken);
            ...
            {
                ReceiveEventType = ReceiveEventType.ParticipateInLottery,//TODO
                TenantId = lottery.Promotion.TenantId,
                PromotionId = lottery.PromotionId,
            ...
            logger.LogInformation("Deducted {Amount} points (PointId: {PointId}) from customerTenant {CustomerId} for lottery {LotteryId}",
                lottery.PaymentAmount.Value, lottery.PaymentPointId.Value, customerTenant.Id, lottery.Id);
        }
        ...
        // If winner, distribute reward
        if (winningReward != null && winningReward.CanDistribute)
        {
            // Update distributed count
            winningReward.DistributedCount++;
            
            // Create RewardAsset for winner if applicable
            if (winningReward.Reward != null)
            {
                try
                {
                    var rewardAssets = await rewardAssetInternalService.Create(
                    ...
                    if (rewardAssets.Count != 0)
                    {
                        participant.RewardAssetId = rewardAssets.First().Id;
                        participant.IsRewardDistributed = true;
                        participant.RewardDistributedAt = DateTime.UtcNow;
                    }
                    
                    logger.LogInformation("Reward distributed to lottery winner: {CustomerTenantId}, Reward: {RewardId}, Quantity: {Amount}",
                        customerTenant.Id, winningReward.RewardId, winningReward.Amount);
                }
                catch (Exception ex)
                {
                    logger.LogError(ex, "Failed to distribute reward to lottery winner");
        ...
        return new SpinLotteryResponse
        {
            ...
            RewardId = winningReward?.RewardId,
            RewardAmount = winningReward?.Amount,
            Message = winningReward != null && winningReward.Reward != null ? $"تبریک! شما برنده شدید: {winningReward.Reward.Title}" : "متأسفانه این بار برنده نشدید"
        };
    }
    ...
    private async Task ExecuteLottery(Lottery lottery, DateTime now, CancellationToken cancellationToken)
    {
        ...
        foreach (var winner in winners)
        {
            try
            {
                ...
                winner.RewardId = winningReward.RewardId;
                winner.RewardAmount = winningReward.Amount;
                winner.AnnouncedAt = now;

                // Distribute reward if applicable
                if (winningReward.CanDistribute && winningReward.Reward != null)
                {
                    try
                    {
                        var rewardAssets = await rewardAssetInternalService.Create(
                            winningReward.Reward,
                            winner.CustomerTenant,
                            null, // eventLogId
                            null, // ruleId
                            null, // actionId
                            winningReward.Amount,
                        ...
                        if (rewardAssets.Count != 0)
                        {
                            winner.RewardAssetId = rewardAssets.First().Id;
                            winner.IsRewardDistributed = true;
                            winner.RewardDistributedAt = now;
                        }
                    ...
                    {
                        logger.LogError(ex, "Failed to distribute reward to winner {ParticipantId}", winner.Id);
                    }
                }
        ...
        logger.LogInformation("Lottery {LotteryId} executed: {WinnerCount} winners selected, {DistributedCount} rewards distributed",
        ...
        List<LotteryReward> rewards)
    {
        ...
        for (int i = 0; i < winnerCount && availableParticipants.Count != 0; i++)
        {
    ...
    private async Task AddPreLotteryParticipants(Lottery lottery, DateTime now, CancellationToken cancellationToken)
    {
        ...
        var lotteryWithSegments = await lotteryRepo.FirstOrDefaultWithIncludeAsync(
            l => l.Promotion.CustomerSegments,
            l => l.Id == lottery.Id,
            cancellationToken);
        ...
        if (lotteryWithSegments.Promotion.CustomerSegments.Count == 0)
        {
        ...
        foreach (var lotterySegment in lotteryWithSegments.Promotion.CustomerSegments)
        {
            ...
            var segmentMemberships = await segmentMembershipRepo.Query()
                .Where(sm => sm.SegmentId == lotterySegment.CustomerSegmentId
                    && sm.CustomerTenant.TenantId == lottery.Promotion.TenantId
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var lotteries = await lotteryRepo.Query()
            .Where(l => l.Promotion.TenantId == request.TenantId
                && l.LotteryType == LotteryType.Scheduled
                && l.IsActive
                && (l.FromDate == null || l.FromDate <= now)
                && (l.ToDate == null || l.ToDate >= now))
            .Include(l => l.Promotion.CustomerSegments)
            .Include(l => l.LotteryRewards)
                .ThenInclude(lr => lr.Reward)
            .ToListAsync(cancellationToken);
        ...
        foreach (var lottery in lotteries)
        {
            ...
            if (lottery.Promotion.CustomerSegments.Count != 0)
            {
                // Customer must be in at least one of the lottery's segments
                canParticipate = lottery.Promotion.CustomerSegments
    ...
    public async Task<List<LotteryWinnerDto>> GetLotteryResultsAsync(int lotteryId, CancellationToken cancellationToken = default)
    {
        ...
        return [.. winners.Select(w => 
        {
            ...
            return new LotteryWinnerDto
            {
                ...
                RewardAmount = w.RewardAmount ?? 0,
                AnnouncedAt = w.AnnouncedAt ?? w.ParticipatedAt,
                IsRewardDistributed = w.IsRewardDistributed
            };
        })];
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return [.. participations.Select(p => new CustomerLotteryHistoryDto
        {
            ...
            IsWinner = p.IsWinner,
            RewardId = p.RewardId,
            RewardTitle = p.Reward?.Title,
            RewardAmount = p.RewardAmount ?? 0,
            AnnouncedAt = p.AnnouncedAt,
            IsRewardDistributed = p.IsRewardDistributed,
            Chance = p.Chance
        })];
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new CustomerLotteryPositionDto
        {
            ...
            RewardId = customerParticipation.RewardId,
            RewardTitle = customerParticipation.Reward?.Title,
            RewardAmount = customerParticipation.RewardAmount ?? 0,
            AnnouncedAt = customerParticipation.AnnouncedAt,
            IsRewardDistributed = customerParticipation.IsRewardDistributed
        };
    }
}
