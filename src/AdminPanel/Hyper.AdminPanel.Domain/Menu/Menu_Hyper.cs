...
public partial class HyperMenuDefinitions : MenuDefinition
{
    private void AddMenu_Hyper()
    {
        ...
        AddMenu("اتصال‌های فروش و حسابداری", "link", "External Integrations", "");
        {
            StartSubMenus();
            AddMenu<ExternalIntegrationConnection>("اتصال‌های سامانه‌ها", "link");
            AddMenu<ExternalProductMapping>("نگاشت کالا و غرفه", "grid-layout");
            AddMenu<IntegrationSyncRun>("سوابق یکسان‌سازی", "refresh");
            EndSubMenus();
        }
