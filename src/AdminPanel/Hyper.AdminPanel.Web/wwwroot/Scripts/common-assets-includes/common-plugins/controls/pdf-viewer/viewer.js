...
_pdfjsLib.PDFJS.maxCanvasHyperels = _pdfjsLib.PDFJS.maxCanvasHyperels === undefined ? 16777216 : _pdfjsLib.PDFJS.maxCanvasHyperels;
...
function getOutputScale(ctx) {
  var deviceHyperelRatio = window.deviceHyperelRatio || 1;
  var backingStoreRatio = ctx.webkitBackingStoreHyperelRatio || ctx.mozBackingStoreHyperelRatio || ctx.msBackingStoreHyperelRatio || ctx.oBackingStoreHyperelRatio || ctx.backingStoreHyperelRatio || 1;
  var HyperelRatio = deviceHyperelRatio / backingStoreRatio;
  return {
    sx: HyperelRatio,
    sy: HyperelRatio,
    scaled: HyperelRatio !== 1
  };
}
...
function normalizeWheelEventDelta(evt) {
  ...
  var MOUSE_DOM_DELTA_HyperEL_MODE = 0;
  var MOUSE_DOM_DELTA_LINE_MODE = 1;
  var MOUSE_HyperELS_PER_LINE = 30;
  var MOUSE_LINES_PER_PAGE = 30;
  if (evt.deltaMode === MOUSE_DOM_DELTA_HyperEL_MODE) {
    delta /= MOUSE_HyperELS_PER_LINE * MOUSE_LINES_PER_PAGE;
...
var PDFPageView = function () {
  ...
  _createClass(PDFPageView, [{
    ...
    value: function update(scale, rotation) {
      ...
      if (this.canvas && _pdfjsLib.PDFJS.maxCanvasHyperels > 0) {
        var outputScale = this.outputScale;
        if ((Math.floor(this.viewport.width) * outputScale.sx | 0) * (Math.floor(this.viewport.height) * outputScale.sy | 0) > _pdfjsLib.PDFJS.maxCanvasHyperels) {
    ...
    value: function paintOnCanvas(canvasWrapper) {
      ...
      if (_pdfjsLib.PDFJS.maxCanvasHyperels > 0) {
        var HyperelsInViewport = viewport.width * viewport.height;
        var maxScale = Math.sqrt(_pdfjsLib.PDFJS.maxCanvasHyperels / HyperelsInViewport);
